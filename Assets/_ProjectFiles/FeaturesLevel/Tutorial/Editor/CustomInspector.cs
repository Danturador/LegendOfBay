		...
		if (selectedObjectProp.objectReferenceValue != null)
		{
			...
			if (types.Count > 0)
			{
				...
				if (selectedTypeIndex >= 0)
				{
					...
					{
						// Обновление индекса для выбранного поля
						...
						if (checker.selectedFieldIndex >= 0)
						{
							...
							if (newSelectedOperatorIndex != checker.selectedOperatorIndex) // Проверка изменения
							{
...
//		// Проверяем, что выбранный объект не null
//		if (selectedObjectProp.objectReferenceValue != null)
//		{
//			GameObject gameObject = (GameObject)selectedObjectProp.objectReferenceValue;
//			MonoBehaviour[] components = gameObject.GetComponents<MonoBehaviour>();
//			List<Type> types = new List<Type>();
...
//						if (checker.selectedFieldIndex >= 0)
//						{
//							var selectedField = fieldNames[checker.selectedFieldIndex];

//							// Обработка логики для поля enabled
//							if (selectedField == "enabled")
//							{
//								MonoBehaviour componentInstance = (MonoBehaviour)((GameObject)selectedObjectProp.objectReferenceValue).GetComponent(selectedType);
//								bool isEnabled = componentInstance != null && componentInstance.enabled;
//								bool newEnabledState = EditorGUILayout.Toggle("Enabled", isEnabled);
//								if (isEnabled != newEnabledState)
//								{
//									componentInstance.enabled = newEnabledState; // Устанавливаем новое состояние
//								}
//							}

//							// Логика для свойств и операторов
//							propertyNames.Clear();
//							if (selectedField != "enabled" && typeof(IEnumerable).IsAssignableFrom(selectedType.GetField(selectedField).FieldType))
//							{
//								propertyNames.Add("Count");
//							}
//							propertyNames.Add("Null");
